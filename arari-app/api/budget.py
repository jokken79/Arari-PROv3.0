...
class BudgetService:
    ...
    def create_budget(self, period: str, entity_type: str = "total",
                      entity_id: str = None, budget_revenue: float = 0,
                      budget_cost: float = 0, budget_profit: float = None,
                      budget_margin: float = None, notes: str = None,
    ...
    def update_budget(self, budget_id: int, **kwargs) -> Dict[str, Any]:
        """Update a budget"""
        allowed_fields = ["budget_revenue", "budget_cost", "budget_profit",
                         "budget_margin", "notes"]
    ...
    def get_budget(self, period: str, entity_type: str = "total",
                   entity_id: str = None) -> Optional[Dict[str, Any]]:
    ...
    def get_budgets(self, period: str = None, entity_type: str = None) -> List[Dict[str, Any]]:
    ...
    def compare_budget_vs_actual(self, period: str,
        ...
        variance_margin = actual_margin - budget["budget_margin"] if actual_margin else None

        # Calculate percentages
        pct_revenue = (variance_revenue / budget["budget_revenue"] * 100) if budget["budget_revenue"] else None
        pct_cost = (variance_cost / budget["budget_cost"] * 100) if budget["budget_cost"] else None
        pct_profit = (variance_profit / budget["budget_profit"] * 100) if budget["budget_profit"] else None
    ...
    def get_budget_summary(self, year: int = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_status": "over_budget" if total_actual_profit < total_budget_profit else "on_track",
    ...
    def delete_budget(self, budget_id: int) -> Dict[str, Any]:
        ...
        self.cursor.execute("DELETE FROM budget_history WHERE budget_id = ?", (budget_id,))
    ...
    def copy_budget_to_period(self, source_period: str, target_period: str,
                              adjustment_pct: float = 0) -> Dict[str, Any]:
        ...
        for row in self.cursor.fetchall():
            ...
            result = self.create_budget(
                ...
                notes=f"Copied from {source_period}" + (f" with {adjustment_pct}% adjustment" if adjustment_pct else "")
            )
