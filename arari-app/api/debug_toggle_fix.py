...
def test_toggle():
    ...
    cursor.execute("SELECT DISTINCT period FROM payroll_records ORDER BY period DESC LIMIT 1")
    row = cursor.fetchone()
    period = row[0] if row else None
    print(f"Using period: {period}")

    stats_before = service.get_statistics(period)
    print(f"Total Profit: {stats_before['total_monthly_profit'] if stats_before else 'None'}")
    ...
    print(f"Total Profit: {stats_after['total_monthly_profit'] if stats_after else 'None'}")
    
    # 5. Verify Difference
    diff = (stats_before['total_monthly_profit'] or 0) - (stats_after['total_monthly_profit'] or 0)
    ...
    if abs(diff - target_profit) < 1.0:
        ...
        print(f"❌ FAILURE: Stats mismatch. Expected difference ~{target_profit}, got {diff}")
