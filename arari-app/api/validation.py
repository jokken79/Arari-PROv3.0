...
class ValidationService:
    ...
    def validate_calculations(self) -> List[ValidationResult]:
        ...
        for row in self.cursor.fetchall():
            ...
            if billing and cost:
                ...
                expected_margin = (
                    (expected_profit / billing) * 100 if billing > 0 else 0
                )
                if (
                    margin and abs(margin - expected_margin) > 0.1
    ...
    def auto_fix(self, fix_types: List[str] = None) -> Dict[str, Any]:
        ...
        return {"fixed": fixed, "failed": failed, "message": f"Fixed {fixed} records"}
